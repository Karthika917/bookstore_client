...
import { sendMsgApi } from '../../services/allApi';
  ...
  return (
    <>
     <Header/>
       ...
       <input type="text" name='name' value={formData.name} onChange={handleChange} className='w-full mt-5 bg-white rounded-sm py-2' placeholder='Name ' />
       <input type="text" name='email' value={formData.email} onChange={handleChange} className='w-full mt-5 bg-white rounded-sm py-2' placeholder='Email ID ' />
       <textarea name="message" id="" value={formData.message} onChange={handleChange} className='w-full  mt-5 bg-white rounded-sm py-2' rows={'8'} placeholder='Message'>
       </textarea>
       <button onClick={handleSubmit} className='bg-black text-white py-4 w-full flex justify-center items-center gap-2'>Send <IoIosSend/></button>
        </div>
